...
void Ext4Read::clear_partitions()
{
    list<Ext4Partition *>::iterator i;
...
list<Ext4Partition *> Ext4Read::get_partitions()
{
    return nparts;
}
...
int Ext4Read::scan_ebr(FileHandle handle, lloff_t base, int sectsize, int disk)
{
    ...
    while(1)
    {
        ...
        if(part->sys_ind == EXT4)
        {
            partition = new Ext4Partition(get_nr_sects(part), get_start_sect(part)+ ebrBase + ebr2, sectsize, handle, NULL);
...
int Ext4Read::scan_gpt(FileHandle handle, lloff_t base, int sectsize, int disk)
{
    unsigned char sector[SECTOR_SIZE];
    ...
    for (i = 0; i < header.num_partitions; i += entries_per_sector)
    {
        ...
        for (j = 0; j < entries_per_sector; j += 1)
        {
            ...
            {
                partition = new Ext4Partition(entry.last_lba - entry.first_lba,
...
int Ext4Read::scan_partitions(char *path, int diskno)
{
   ...
   // part = pt_offset(sector, 0);
    ...
    for(i = 0; i < 4; i++)
    {
        ...
        if((part->sys_ind != 0x00) || (get_nr_sects(part) != 0x00))
        {
            ...
            if(part->sys_ind == EXT4)
            {
                partition = new Ext4Partition(get_nr_sects(part), get_start_sect(part), sector_size, handle, NULL);
                if(partition->is_valid)
                {
...
Ext4File* Ext4Read::add_loopback(const char *file)
{
    int ret, sector_size;
    Ext4Partition *partition;
    FileHandle handle;
   ...
   if(ret == -INVALID_TABLE)
   {
       ...
       partition = new Ext4Partition(0, 0, sector_size, handle, NULL);
       if(partition->is_valid)
       {
            partition->set_image_name(file);
            nparts.push_back(partition);
            LOG("Linux Partition found on disk %d partition %d\n", ndisks, 0);
            return partition->get_root();
        }
       else
       {
           delete partition;
