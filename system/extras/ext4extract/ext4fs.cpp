...
Ext4Partition::Ext4Partition(lloff_t size, lloff_t offset, int ssize, FileHandle phandle, const char *vol)
{
    int ret;
...
Ext4Partition::~Ext4Partition()
{
    free (desc);
}

void Ext4Partition::set_linux_name(const char *name, int disk, int partition)
{
    char dchar = 'a' + disk;
    char str_buffer[10];
...
string &Ext4Partition::get_linux_name()
{
    return linux_name;
}

string &Ext4Partition::get_volume_name()
{
    return volume_name;
}

int Ext4Partition::ext4_readblock(lloff_t blocknum, void *buffer)
{
    char *newbuffer = NULL;
...
int Ext4Partition::mount()
{
    ...
    if(sblock.s_magic != EXT4_SUPER_MAGIC)
    {
        LOG("Bad Super Block. The drive %s is not ext4 formatted.\n", linux_name.c_str());
        return -1;
    }

    if(sblock.s_feature_incompat & EXT4_FEATURE_INCOMPAT_COMPRESSION)
    {
        LOG("File system compression is used which is not supported.\n");
    }
    blocksize = EXT4_BLOCK_SIZE(&sblock);
    inodes_per_group = EXT4_INODES_PER_GROUP(&sblock);
    inode_size = EXT4_INODE_SIZE(&sblock);

    volume_name = sblock.s_volume_name;

    LOG("Block size %d, inp %d, inodesize %d\n", blocksize, inodes_per_group, inode_size);
    totalGroups = (sblock.s_blocks_count)/EXT4_BLOCKS_PER_GROUP(&sblock);
    gSizeb = (sizeof(EXT4_GROUP_DESC) * totalGroups);
    gSizes = (gSizeb / sect_size)+1;

    desc = (EXT4_GROUP_DESC *) calloc(totalGroups, sizeof(EXT4_GROUP_DESC));
    if(desc == NULL)
    {
...
EXT4DIRENT *Ext4Partition::open_dir(Ext4File *parent)
{
    EXT4DIRENT *dirent;
...
Ext4File *Ext4Partition::read_dir(EXT4DIRENT *dirent)
{
    string filename;
    Ext4File *newEntry;
    char *pos;
    ...
    if(!dirent->dirbuf)
    {
        dirent->dirbuf = (EXT4_DIR_ENTRY *) new char[blocksize];
        if(!dirent->dirbuf)
            return NULL;
    ...
    else
    {
        ...
        dirent->next = (EXT4_DIR_ENTRY *)(pos + dirent->next->rec_len);
...
void Ext4Partition::close_dir(EXT4DIRENT *dirent)
{
    delete [] dirent->dirbuf;
    delete dirent;
}

Ext4File *Ext4Partition::read_inode(uint32_t inum)
{
    ...
    if(blknum != last_block) {
        ret = ext4_readblock(blknum, inode_buffer);
        if (ret < 0) {
    ...
    src = (EXT4_INODE *)(inode_buffer + inode_index);
    file->inode = *src;
    ...
    file->partition = (Ext4Partition *)this;
    file->onview = false;
...
int Ext4Partition::read_data_block(EXT4_INODE *ino, lloff_t lbn, void *buf)
{
    lloff_t block;
    ...
    return ext4_readblock(block, buf);
}

lloff_t Ext4Partition::extent_binarysearch(EXT4_EXTENT_HEADER *header, lloff_t lbn, bool isallocated)
{
    EXT4_EXTENT *extent;
    ...
    for(int i = 0; i < header->eh_entries; i++)
    {
           ...
           (lbn < (index + 1)->ei_block))
        {
            ...
            ext4_readblock(block, (void *) child);
...
lloff_t Ext4Partition::extent_to_logical(EXT4_INODE *ino, lloff_t lbn)
{
    lloff_t block;
...
uint32_t Ext4Partition::fileblock_to_logical(EXT4_INODE *ino, uint32_t lbn)
{
    ...
    if((lbn >= EXT4_NDIR_BLOCKS) && (lbn < indlast))
    {
        block = ino->i_block[EXT4_IND_BLOCK];
        ext4_readblock(block, indbuffer);
    ...
    if((lbn >= indlast) && (lbn < dindlast))
    {
        ...
        ext4_readblock(block, dindbuffer);
    ...
    if(lbn >= dindlast)
    {
        ...
        ext4_readblock(block, tindbuffer);
